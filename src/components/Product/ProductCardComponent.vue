...
<template>
    <div v-for="(item,index) in products" key="index" class="w-72 bg-white shadow-md rounded-xl duration-500 hover:scale-105 hover:shadow-xl">
          <router-link :to="'/product/'+item.id">
            <img :src="item.thumbnail" alt="Product" class="h-80 w-72 object-cover rounded-t-xl" />
            <div class="px-4 py-3 w-72">
              ...
              <div class="flex items-center">
                <p class="text-lg font-semibold text-black cursor-auto my-3">${{(item.price - (item.price * (item.discountPercentage / 100))).toFixed()}}</p>
                <del>
